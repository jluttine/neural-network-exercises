...
\item The Levenberg-Marquardt algorithm is
  \[
  \vect{w}(n+1) = \vect{w}(n) - \left(\matr{J}^T(n)\matr{J}(n) +
  ...
  \begin{solution}
    ...
    \]
    which is the Levenberg-Marquardt learning rule.  When $\sigma$ is
    close to zero, the method is close to the Gauss-Newton
    method. When $\sigma$ is large, the method is close to the
    steepest descent method.  The parameter $\sigma$ can be varied
    during the iteration in order to achieve efficient convergence. A
